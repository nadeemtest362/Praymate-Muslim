-- Simplify prayer system with clear separation between onboarding and daily prayers
-- 1. Onboarding prayers are ONLY "First Prayer", never count as morning/evening
-- 2. Simple 4am-4pm morning, 4pm-4am evening periods
-- 3. Day boundary at 4am
...
BEGIN
    ...
    -- Get onboarding prayer (separate logic)
    SELECT 
        JSONB_BUILD_OBJECT(
            'id', id,
            'content', content,
    ...
    -- Show onboarding prayer if:
    -- 1. It exists
    -- 2. It was created within the last 4 hours OR we're still in the first prayer window
    IF onboarding_prayer IS NOT NULL THEN
        DECLARE
            onboarding_created_at TIMESTAMPTZ;
            hours_since_onboarding NUMERIC;
        BEGIN
            onboarding_created_at := (onboarding_prayer->>'generated_at')::TIMESTAMPTZ;
            hours_since_onboarding := EXTRACT(EPOCH FROM (NOW() - onboarding_created_at))/3600;
            
            -- Show for 4 hours or until user has both morning and evening prayers
            IF hours_since_onboarding <= 4 OR (morning_prayer IS NULL AND evening_prayer IS NULL) THEN
...
- Onboarding prayers are completely separate from daily prayers
